...
**AgentTaskboardManifest (ATM)** 旨在定义一套让 Agent 学习、执行和自我总结的标准工作流语言。

* **String-heavy（语义化优于结构化）：** 尽可能使用 `<string>` 自然语言描述动作和约束，将解析工作交给 Agent 的大脑，而非硬编码的规则引擎。
* **Lazy Load（懒加载与极简上下文）：** 避免一次性将所有任务塞给 Agent 导致 Token 溢出或幻觉。通过入口文件和子任务的动态调用，维持极简的执行上下文。
* **Unknown Tolerance（未知容忍）：** 任何在设计时无法确定的字段，显式填入 `<unknowns>`。这是一种授权机制，提示 Agent 在运行时动态推断或向用户发起询问。
* **Human-in-the-loop Parity（人机同权）：** 将“呼叫人类协助”视为与“调用工具”或“执行脚本”同等的标准 Action，无缝融入状态机路由。
...
**相对引用约定：**
任务间的相互调用必须使用相对路径（如 `./xxx_xx.task.yaml`），以保证整个工作流模块的极强可移植性。

## Schema Definition

ATM 规范摒弃了繁琐的类型字段，采用**“键名作为锚点，字符串包揽约束”**的极简映射规则。
...
global_inputs:                 # 全局输入约束 (Key 为变量名，Value 为自然语言约束)
  ...
  <input_key>: "<string>"      # 举例: target_molecule: "选填。目标分子的 SMILES，默认为 unknowns。"
...
inputs:                        # 任务输入列表
  <param_key>: "<string>"      # 举例: gjf_file: "必填。待处理的 Gaussian 输入文件，来自上游，.gjf 格式。"
outputs:                       # 任务输出列表
...
ATM 采用**按顺序匹配，命中即停 (Short-circuit evaluation)** 的动态路由机制。在 `<step>` 的 `when` 模块中，`<response>` 仅包含以下 5 种核心指令，分为“生命周期”与“流程跳转”两类：
...
global_inputs:
  target_species: "必填。目标分子的名称或 SMILES 字符串。"
  slurm_partition: "选填。目标计算节点队列，默认为 'cpu'。"
global_outputs:
  final_log_file: "Gaussian 优化完成后的 .log 文件路径。"
global_conventions:
...
inputs:
  species: "必填。继承自全局输入 target_species。"
steps:
