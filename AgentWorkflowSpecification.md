...
* **Tree-structured Orchestration（树形编排与细节下沉）：** 上层任务偏向负责宏观逻辑调度与状态流转，具体的执行细节应该放到更下层独立的子 `*.task.yaml` 中。整个工作流构成一棵“自由生长”的任务树，而不是一大片低矮的草丛。
* **Conciseness（简洁至上）：** AI 按照标准生成或总结的工作流方案必须是极度简洁的，去除一切不必要的嵌套（如一个一步的任务是调用另一个任务）和冗余字段，直击任务核心。
